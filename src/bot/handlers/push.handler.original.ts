...
export class PushHandler {
  ...
  private async getUserPushSettings(userId: string): Promise<{
    ...
    try {
      ...
      if (!accessToken) {
        // 返回默认设置而不是抛出错误
        logger.warn('No cached access token found for user', { userId: parseInt(userId || '0') });
        return {
          settings: {
            flash_enabled: false,
            whale_enabled: false,
      ...
      // 如果是认证错误(401/403)，返回默认设置
      if (error instanceof ApiError && (error.status === 401 || error.status === 403)) {
        logger.warn('Authentication failed, returning default settings', { userId: parseInt(userId || '0') });
        return {
          settings: {
            flash_enabled: false,
            whale_enabled: false,
      ...
      // 其他错误也返回默认设置，避免整个界面失效
      logger.warn('API error, returning default settings', { 
        userId: parseInt(userId || '0'),
        error: (error as Error).message
      });
      return {
  ...
  public async handleCallback(ctx: ExtendedContext, callbackData: string): Promise<void> {
    ...
    try {
      ...
      if (errorMessage.includes('用户未认证') || errorMessage.includes('身份验证')) {
        await ctx.answerCbQuery('⚠️ 请先完成身份验证，然后重试');
      } else {
  ...
  private async updateUserPushSetting(userId: string, type: string, enabled: boolean): Promise<void> {
    try {
      ...
      const accessToken = await getUserToken(userId);
      if (!accessToken) {
        logger.warn('No access token found for push setting update', { userId: parseInt(userId || '0'), type, enabled });
        throw new Error('用户未认证，请先完成身份验证');
      }
